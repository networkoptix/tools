...
class CommandResults(object):
    ...
    def __repr__(self):
        return 'exit_code=%d stdout=[%s] stderr=[%s]' % (self.exit_code, self.stdout, self.stderr)
...
class Host(object):
    ...
    def get_command_output(self, args, input=None, cwd=None, env=None, check_retcode=True, log_output=True, timeout=None):
        results = self.run_command(args, input, cwd, env, check_retcode, log_output, timeout)
...
class LocalHost(Host):
    ...
    def run_command(self, args, input=None, cwd=None, env=None, check_retcode=True, log_output=True, timeout=None, merge_stderr=False):
        ...
        pipe = subprocess.Popen(
            ...
            stdin=stdin,
            close_fds=is_unix,  # close on unices
...
class RemoteSshHost(Host):
    ...
    def file_exists(self, path):
        output = self.get_command_output(['[', '-f', path, ']', '&&', 'echo', 'yes', '||', 'echo', 'no']).strip()
        assert output in ['yes', 'no'], repr(output)
        return output == 'yes'

    def dir_exists(self, path):
        output = self.get_command_output(['[', '-d', path, ']', '&&', 'echo', 'yes', '||', 'echo', 'no']).strip()
        assert output in ['yes', 'no'], repr(output)
        return output == 'yes'
    ...
    def read_file(self, from_remote_path, ofs=None):
        if ofs:
            ...
            return self.get_command_output(['tail', '--bytes=+%d' % ofs, from_remote_path], log_output=False)
        else:
            return self.get_command_output(['cat', from_remote_path], log_output=False)
    ...
    def get_file_size(self, path):
        return self.get_command_output(['stat', '--format=%s', path])
    ...
    def expand_path(self, path):
        ...
        return self.get_command_output(['echo', path]).strip()
    ...
    def expand_glob(self, path_mask, for_shell=False):
        ...
        if for_shell:
            ...
            output = self.get_command_output(['echo', path_mask]).strip()
            if output == path_mask:
