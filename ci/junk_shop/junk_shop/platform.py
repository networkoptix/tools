...
class LinuxPlatform(PosixPlatform):
    ...
    def extract_core_source_binary(self, core_path):
        ...
        if not mo:
            print 'Error extracting core source binary from %s: none returned' % core_path
            return None
        return mo.group(1)
...
class DarwinPlatform(PosixPlatform):
    ...
    def extract_core_backtrace(self, binary_path, core_path):
        print 'Extracting backtrace from %s, generated by %s' % (core_path, binary_path or 'unknown binary')
