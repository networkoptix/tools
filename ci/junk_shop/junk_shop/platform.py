...
class LinuxPlatform(PosixPlatform):
    ...
    def extract_core_source_binary(self, core_path):
        ...
        try:
            ...
            print 'Error extracting core file source binary from %s: %s' % (core_path, x)
            print x.output
            return None
        mo = re.match(r".*, from '(\S+).*'", output.rstrip())
        if not mo:
            print 'Error extracting core file source binary from %s: none returned' % core_path
            return None
        return mo.group(1)

    def extract_core_backtrace(self, binary_path, core_path):
        if not binary_path: return
        print 'Extracting core file backtrace from %s, generated by %s' % (core_path, binary_path)
...
class DarwinPlatform(PosixPlatform):
    ...
    def extract_core_source_binary(self, core_path):
        try:
            ...
            print 'Extracting core file source with command: %s' % subprocess.list2cmdline(args)
    ...
    def extract_core_backtrace(self, binary_path, core_path):
        print 'Extracting core file backtrace from %s, generated by %s' % (core_path, binary_path or 'unknown binary')
        args = ([self._lldb_path, '--batch', '--core', core_path] +
                ['--one-line=%s' % command for command in self.LLDB_BACKTRACE_EXTRACT_COMMANDS])
        print 'Extracting core file backtrace with command: %s' % subprocess.list2cmdline(args)
        return subprocess.check_output(args, stderr=subprocess.STDOUT)
