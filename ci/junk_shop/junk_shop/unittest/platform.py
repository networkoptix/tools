"""
  junk_shop.unittest.platform
  ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

  Unit test platform support (specific utils for unittest)
...
class Platform(object):
    """The base abstract class containing specific platform utils for unittests"""
    ...
    @abc.abstractmethod
    def collect_core_file_list(self, test_name, test_info, test_work_dir):
        pass
...
class WindowsPlatform(Platform):
    """Windows specific platform utils"""

    # The windows dump file may be created after the the test execution finishing.
    # To find those files we have to increase search interval -> [started_at, started_at + duration + delay).
    ...
    def collect_core_file_list(self, test_name, test_info, test_work_dir):
        """Returns windows dmp-file path generator for the test by `test_name`, `test_info`.
        The default dump file location is `LOCALAPPDATA`, if we aren't
        use DumpFolder setting (https://docs.microsoft.com/en-us/windows/desktop/wer/collecting-user-mode-dumps).
    ...
    def extract_core_backtrace(self, binary_path, core_path):
        """Returns a backtrace as a string for given `binary_path` and `core_path`.
        """
        if not binary_path:
            return
        log.info('Extracting dump file backtrace from %s, generated by %s', core_path, binary_path)
        return extract_backtrace_from_dump(core_path, binary_path)
...
class PosixPlatform(Platform):
    ...
    def collect_core_file_list(self, test_name, test_info, test_work_dir):
        """Returns core-file paths generator for the test by `test_name`.
         We try to find core files in the `test_work_dir`.
         """
        for path in test_work_dir.rglob(self.CORE_PATTERN):
...
class DarwinPlatform(PosixPlatform):
    """Darwin specific platform utils"""
