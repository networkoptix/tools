...
def clean_build_directory(build_dir: Path,
        ...
        additional_known_files: Optional[Set[str]] = None) -> None:
...
ParsedScriptData = namedtuple("ParsedScriptData", [
    'strengthened_targets', 'commands_to_run', 'do_clean', 'verify_globs_path', 'known_files',
    'changed_files_list_file_name', 'source_dir', 'affected_targets_list_file_name'])
...
def run_script(build_dir: Path, script_file_name: str, force_patch: bool) -> None:
    ...
    if script_data.do_clean:
        clean_build_directory(build_dir=build_dir, build_file_processor=build_file_processor,
            additional_known_files=script_data.known_files)
...
def _parse_script_data(script_file_name: str) -> ParsedScriptData:
    ...
    changed_files_list_file_name = None
    known_files = set()
        ...
        for line in script_file:
            ...
            if command == "strengthen":
                ...
                if len(args) != 1:
                    ...
                    continue
                verify_globs_path = Path(args[0])
            elif command == "generate_affected_targets_list":
                if len(args) != 3:
                ...
                known_files = {changed_files_list_file_name, affected_targets_list_file_name}
            else:
    ...
    return ParsedScriptData(
        ...
        do_clean = do_clean,
        verify_globs_path = verify_globs_path,
        changed_files_list_file_name = changed_files_list_file_name,
        affected_targets_list_file_name = affected_targets_list_file_name,
