...
def _test_jira():
    with JiraFixture() as jira:
        jira.create_issue('server--4.2.0.34407-abc-default--linux-x64--a.gdb-bt')
        jira.update_issue(['server--4.2.0.34407-abc-default--linux-x64--a.gdb-bt'])
        ...
        assert {'4.2'} == jira.field_set('versions')
        assert {'4.2_patch', '5.0'} == jira.field_set('fixVersions')
        ...
        jira.update_issue(['server--5.0.0.35062-tricom-default--linux-x86--c.gdb-bt'])
        assert 'Closed' == jira.api._jira.issue(jira.issue.key).fields.status.name
        ...
        jira.update_issue(['server--4.2.0.34420-abc-default--arm-bpi--b.gdb-bt'])
        assert 'Closed' == jira.issue.fields.status.name
        assert {'a'} == jira.attachments()

        logger.info('Reopen is for new version')
        jira.update_issue(['server--5.0.0.35062-uvi-tricom--linux-x86--c.gdb-bt'])
        assert 'Open' == jira.issue.fields.status.name
        assert {'4.2', '5.0'} == jira.field_set('versions')
        assert {'4.2_patch', '5.0'} == jira.field_set('fixVersions')
        assert {'a', 'c'} == jira.attachments()

        logger.info('No new fix version if the newer ones are set')
        jira.update_issue(['server--4.1.0.32783-abc-default--linux-x64--h.gdb-bt'])
        assert 'Open' == jira.issue.fields.status.name
        assert {'4.1', '4.2', '5.0'} == jira.field_set('versions')
        assert {'4.2_patch', '5.0'} == jira.field_set('fixVersions')
        ...
        jira.issue.update(fields=dict(customfield_11120=35064))
        assert 'Closed' == jira.api._jira.issue(jira.issue.key).fields.status.name

        logger.info('No reopen should happen for report on changeset before fix')
        jira.update_issue(['server--5.0.0.35063-uvi-tricom--arm-rpi--d.gdb-bt'])
        assert 'Closed' == jira.issue.fields.status.name
        assert {'a', 'c', 'h'} == jira.attachments()

        logger.info('Reopen is for new changeset')
        jira.update_issue(['server--5.0.0.35064-xyz-default--arm-rpi--e.gdb-bt'])
        assert 'Open' == jira.issue.fields.status.name
        assert {'4.1', '4.2', '5.0'} == jira.field_set('versions')
        assert {'4.2_patch', '5.0'} == jira.field_set('fixVersions')
        ...
        jira.update_issue(['server--5.0.0.35065-xyz-default--arm-rpi--f.gdb-bt'])
        assert {'4.1', '4.2', '5.0'} == jira.field_set('versions')
        assert {'Future'} == jira.field_set('fixVersions')
        ...
        jira.update_issue(['server--5.1.0.35444-abc-default--linux-x86--g.gdb-bt'])
        assert {'4.1', '4.2', '5.0', '5.1'} == jira.field_set('versions')
        assert {'Future'} == jira.field_set('fixVersions')
