...
class FTReport(Report):
  ...
  def _get_tests_results(self, prev_run = None):
    tasks = self.find_task('Run functional test old > %run_func_tests_old.taskbot%', prev_run)
    if not tasks:
      return []
  ...
  def __get_cores(self):
    tasks = self.find_task('Run functional test old > %run_func_tests_old.taskbot%')
