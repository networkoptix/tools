...
class RRRtspTcpBasic:
    ...
    def __init__(self, server_addr, mac, cid, sid, uname, pwd, urlGen, lock = None,log = None):
        self._addr, port = server_addr
        self._port = int(port)
        self._urlGen = urlGen
...
class SingleServerRtspTestBase:
    ...
    def _fetchCameraList(self):
        ...
        json_obj = SafeJsonLoads(response.read(), self._serverEndpoint, 'getCamerasEx')
        ...
        for c in json_obj:
            ...
            if c['status'] in ('Online', 'Recording'):
                #print "Add camera %s %s %s: %s" % (c['physicalId'], c['id'], c['name'], c['status'])
...
class SingleServerRtspPerf(SingleServerRtspTestBase):
    ...
    def __init__(self,archiveMax,archiveMin,serverEndpoint,guid,username,password,timeoutMax,timeoutMin,threadNum,flag,lock):
        ...
        self._rtspCloseTimeout = flag.rtspCloseTimeout
        # Initialize the performance log
        l = serverEndpoint.split(":")
        self._perfLog = open("%s_%s.perf.rtsp.log" % (l[0],l[1]),"w+")


    # Read some bytes from the socket, no more then RATE bytes per second, no longer then TIMEOUT
    def _timeoutRecv(self, socket, rate, timeout):
        ...
        try:
            while time.time() < finish:
                ...
                if ready[0]:
                    d = socket.recv(1024*16) if rate < 0 else socket.recv(rate)
    ...
    def _dumpArchiveHelper(self,c,tcp_rtsp,timeout,dump,rate):
        #print "Starting _dumpArchiveHelper with timeout %s" % timeout
        finish = time.time() + timeout
        while time.time() < finish:
            try:
                data = self._timeoutRecv(tcp_rtsp._socket,rate,self._rtspTimeout)
                if dump is not None:
                    dump.write(data)
                    dump.flush()
            except:
                ...
                if data == None:
                    with self._lock:
                        ...
                        self._perfLog.write("The RTSP/RTP url:%s %s seconds not response with any data\n" % (tcp_rtsp._url, self._rtspTimeout))
            ...
            #if not self._exitFlag.isOn():
                #with self._lock:
                #    print "--------------------------------------------"
                #    print "The RTP sink interrupted by user on RTSP url %s" % tcp_rtsp._url
            #    return
    ...
    def _dumpStreamHelper(self,c,tcp_rtsp,timeout,dump,rate):
        #print "Starting _dumpStreamHelper with timeout %s" % timeout
        finish = time.time() + timeout
        while time.time() < finish:
            try:
                data = self._timeoutRecv(tcp_rtsp._socket,rate,self._rtspTimeout)
    ...
    def _threadMain(self,dump,num):
        #print "DEBUG: thread %s start" % num
        while self._exitFlag.isOn():
            # choose a random camera in the server list
            c = self._cameraList[random.randint(0,len(self._cameraList) - 1)]
            if random.randint(0,1) == 0:
                #print "(DEBUG: [%s] SingleServerRtspPerf tests _main_streaming on camera %s %s)" % (num, c[2], c[0])
                self._main_streaming(c,dump)
                #print "(DEBUG: [%s] _main_streaming ends.)" % num
                time.sleep(0.1)
            else:
                #print "(DEBUG: [%s] SingleServerRtspPerf tests _main_archive on camera %s %s)" % (num, c[2], c[0])
                self._main_archive(c,dump)
                #print "(DEBUG: [%s] _main_archive ends)" % num
                time.sleep(0.1)
        #print "DEBUG: thread %s end" % num
    ...
    def run(self):
        ...
        for _ in xrange(self._threadNum):
            th = threading.Thread(target=self._threadMain,args=(dump,_))
            th.start()
...
class RtspPerf:
    ...
    def _loadConfig(self):
        ...
        self.rtspCloseTimeout = config_parser.getint("Rtsp", "rtspCloseTimeout")
        ...
        for i in xrange(len(clusterTest.clusterTestServerList)):
            ...
            #response = urllib2.urlopen("http://%s/api/discoveredPeers?showAddresses=true" % (serverAddr))

            #if response.getcode() != 200:
            #    raise Exception("Cannot connect to server:%s using discoveredPeers" % serverAddr)
            #json_obj = SafeJsonLoads(response.read(), serverAddr, 'discoveredPeers')
            #if json_obj['reply']:
            #    for peer in json_obj['reply']:
            #        print "Peer: system %s, name %s, addresses %s" % (peer['systemName'], peer['name'], peer['remoteAddresses'])
