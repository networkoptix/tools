        ...
        with cls._timerLock:
            if url in cls._globalTimerTable:
                if cls._globalTimerTable[url] > cls.HALF_MAX_TIMEOUT:
                    ...
                    cls._globalTimerTable[url] *= cls._globalTimerTable[url] * 2.0
            else:
...
class SingleServerRtspTestBase(object):
    ...
    def _fetchCameraList(self):
        ...
        for c in obj:
            ...
            self._allCameraList.append(Camera(c["physicalId"],c["id"],c["name"].encode('utf8'),c['status']))
            if c['status'] in ('Online', 'Recording'):
                self._cameraList.append(self._allCameraList[-1])
        ...
        if not self._cameraList:
            print "Error: no active cameras found on server %s" % (self._serverAddr,)
...
class FiniteSingleServerRtspTest(SingleServerRtspTestBase):
    ...
    def _testMain(self):
        ...
        if self._cameraList:
            ...
            with RRRtspTcpBasic(l[0:2], c[0:2],
                         self._serverGUID,
                         self._username,
                         self._password,
...
class SingleServerRtspPerf(SingleServerRtspTestBase):
    ...
    def _main_streaming(self, c):
        l = self._serverAddr.split(':')
        obj = RRRtspTcpBasic(l[0:2], c[0:2],
                     self._serverGUID,
                     self._username, self._password,
...
class RtspPerf(object):
    ...
    def run(self, need_dump=False):
        ...
        print "---------------------------------------------"
        #self._cluster.unittestRollback.removeRollbackDB()
