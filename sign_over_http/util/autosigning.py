...
# This script enters password for hardware signing token.
# It is supposed to be autorun on user autologin.
# It enters password every time when authentication dialogue appears.
...
def clicker_thread(password, entered_count):
    while True:
        try:
...
print('')
print('Start to check the auth dialogue at: %s (UTC+%d)' % (time.asctime(), -time.timezone/3600))
key_password = os.environ['KEY_PASSWORD']  # key is missing if not added from credentials

entered_count = [0]
clicker = threading.Thread(target=clicker_thread, args=(key_password, entered_count))
clicker.start()
