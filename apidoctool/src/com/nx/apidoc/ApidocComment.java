...
public abstract class ApidocComment
{
    ...
    protected static final String TAG_EXAMPLE = "%example";
    protected static final String TAG_JSONRPC = "%jsonrpc";
...
//
// Apidoc comments are C++ comments from which the Rest API documentation is generated by
...
//
// If an API function's input, result, or a certain param is an object or an object array, the
// fields and apidoc comments from its C++ struct can be embedded by specifying the
// "%struct StructName" tag instead of or additionally (prepending) to "%param" tags for the
// object fields. If "%param" tags are specified after the "%struct" tag, they override comments
// coming from the struct. If the "%struct" tag follows some "%param" tag, it is considered to
// describe the inner fields of that "%param".
//
...
// that the item must be omitted from the generated documentation.
//
// Attribute "[proprietary]" may appear in "%param", or function's "%apidoc", where it means that
// the item must be hidden from the user when the generated documentation is presented in the
// browser, but should technically be present in that documentation source format; also it may
// appear in "%value" meaning that the item must be completely ignored.
//
...
// This line inserts into the documentation a hard-coded text which describes the "format"
// parameter which is common to many API functions and defines the format (JSON, UBJSON, etc.) of
// the requested data.
}
