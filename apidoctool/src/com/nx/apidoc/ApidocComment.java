...
// Apidoc comment format
    ...
    // - Introduction
    //
    // Apidoc comment example: see in test/source_code_parser.
    //
    ...
    // Header ::= S* "/**"
    // FunctionHeader ::= "%apidoc" (S* ":" S* "arrayParams")? (S* "[" TagAttr "]")? \
    //     S ("GET" | "POST") S MethodName
    // TypeHeader ::= "%apidoc" (S* "[" TagAttr "]")?
    // TagName ::= [a-z]+ //< See ApidocComment.TAG_*
    // Label ::= [A-Za-z]+ //< See enum Apidoc.Type.toString()
    // TagAttr ::= [a-z]+ //< See ApidocComment.ATTR_*
    // Tag ::= S* "*" S+ "%" TagName (S* "[" TagAttr "]")? (S* ":" S* Label)? \
    //     S* (S Char+ (NewLine TextLine)*)?
    // Footer ::= S* "*/"
    // ApidocComment ::= Header ((FunctionHeader | TypeHeader) \
    //     (NewLine TextLine)* (NewLine Tag)*)+ Footer
    //
    // - Semantics of labels
    //
    ...
    //
    // Attribute "[unused]" may appear only in "%param" It means that the param will be omitted
