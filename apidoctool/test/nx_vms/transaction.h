 ...
 * false,                   -- The transaction is not persistent (does not save anything to the
 *                             database).
 * false,                   -- The transaction is not system (handled in a common way).
 * InvalidGetHashHelper(),  -- Calculates hash for a persistent transaction.
 ...
 *                             implement the second stage of transaction processing "in memory,
 *                             non-db" logic (e.g. work with the Resource Pool). It's quite
 *                             possible that a non-persistent transaction triggers some
 *                             notifications (for example, dumpDatabase).
 * InvalidAccess(),         -- Actual only for persistent transactions with one element. The
 *                             warning below MUST be fulfilled.
 * InvalidAccess(),         -- Actual only for read transactions with one element. The warning
 *                             below MUST be fulfilled.
 * InvalidFilterFunc(),     -- Actual only for persistent transactions with an element list. The
 *                             warning below MUST be fulfilled.
 * FilterListByAccess<ShowreelAccess>(),
 *                          -- Filter the requested list by the specified checker.
 *                             The warning below MUST be fulfilled.
 * AllowForAllAccessOut(),  -- Actual only for persistent transactions. Desides if the remote peer
 *                             has enough rights to receive this transaction during the proxy
 *                             transaction stage.
 ...
 * ShowreelNotificationManagerHelper(),
 *                          -- Notify other users that we have changed the Showreel.
 ...
 *                          -- Resending the persistent transactions.
 * RegularTransactionType() -- The transaction is regular, without any magic.
 * )
 *
 * WARNING:
...
APPLY(201, removeResource, nx::vms::api::IdData, \
    ...
    AllowForAllAccessOut(), /*< check remote peer rights for outgoing transaction */ \
    RegularTransactionType()) /*< regular transaction type */ \
/**%apidoc Description of Transaction #202 setResourceStatus. */ \
...
/**%apidoc
 * Multiline description for Transaction removeResourceStatus #213.
...
APPLY(300, getCameras, nx::vms::api::CameraDataList, \
    ...
    RegularTransactionType()) /*< regular transaction type */ \
/**%apidoc
 * %param url Redefining the description of the "url" parameter.
 */ \
