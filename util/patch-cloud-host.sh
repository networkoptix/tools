...
FILE_NAME="libcommon.so"

# From C++ source:
# static const char* kCloudHostNameWithPrefix =
...
find_vms_dir()
{
    ...
    if [ "$VMS_DIR" = "/" ]; then
        error "Run this script from any dir inside nx_vms."
    fi
}
...
if [ $# = 0 -o $# -gt 2 -o "$1" = "-h" -o "$1" = "--help" ]; then
    echo "Utility to patch the libcommon.so binary in order to replace Cloud Host."
    echo
    echo "Usage:"
    echo
    echo "Show current value:"
    echo "$(basename $0) -show [path/to/libcommon.so]"
    echo
    echo "Patch with new value:"
    echo "$(basename $0) 'new_cloud_group' [path/to/libcommon.so]"
    exit 0
fi
...
if [ $# -ge 2 ]; then
    ...
    find_array "$VMS_DIR/$FILE_LOCATION" -name "$FILE_NAME"

    # Make sure 'find' returned exactly one file.
    if [ ${#FILES[*]} = 0 ]; then
        error "Unable to find $FILE_NAME in $FILE_LOCATION." >&2
    fi
    if [ ${#FILES[*]} -gt 1 ]; then
        error "Found ${#FILES[*]} files named $FILE_NAME in $FILE_LOCATION." >&2
    fi
    FILE=${FILES[0]}
fi
...
if [ -z "$STRING" ]; then
    error "'$CLOUD_HOST_KEY' string not found in $FILE"
fi

OFFSET=$(echo "$STRING" |awk '{print $1}')
EXISTING_CLOUD_HOST=$(echo "$STRING" |awk '{print $3}')

#echo "DEBUG: Offset: $OFFSET, existing cloud host: $EXISTING_CLOUD_HOST"
...
if [ -z "$NEW_CLOUD_HOST" ]; then
    ...
    #echo "DEBUG: Patch offset: $PATCH_OFFSET, NUL offset: $NUL_OFFSET"

    # Patching the printable chars.
    echo -ne "$NEW_CLOUD_HOST" |dd bs=1 conv=notrunc seek="$PATCH_OFFSET" of="$FILE" 2>/dev/null || error "dd 1 failed"

    # Placing '\0' at the end of the string.
    dd bs=1 conv=notrunc seek="$NUL_OFFSET" of="$FILE" if=/dev/zero count=1 2>/dev/null || error "dd 2 failed"
